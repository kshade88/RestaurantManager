...
from bar.services import get_available_quantity_in_milliliters, consume_product_inventory, get_available_skus, get_sku_available_quantity_in_milliliters, calculate_sku_consumption

class BarConsumptionServicesTestCase(TestCase):
    def setUp(self):
    ...
    def test_get_available_quantity_in_milliliters(self):
        available_quantity = get_available_quantity_in_milliliters(self.stock_item.product)
    ...
    def test_get_sku_available_quantity_in_milliliters(self):
        sku_quantity = get_sku_available_quantity_in_milliliters(self.stock_item)
        self.assertEqual(sku_quantity, 7500)  # 750 ml * 10 quantity
        sku_quantity2 = get_sku_available_quantity_in_milliliters(self.stock_item2)
    ...
    def test_consume_product_inventory_consuming_first_sku_only(self):
        initial_quantity = get_available_quantity_in_milliliters(self.stock_item.product)
        consume_product_inventory(self.stock_item.product, 100)
        remaining_quantity = get_available_quantity_in_milliliters(self.stock_item.product)
